use chacha20poly1305::{Key, KeyInit, XChaCha20Poly1305, aead::OsRng};
use rusqlite::Connection;
use uuid::Uuid;

use crate::crypto::{VaultKeys, decrypt_payload, encrypt_payload};
...
impl EncryptedVault {
    pub fn decrypt_name(&self, keys: &VaultKeys) -> Result<String, Box<dyn std::error::Error>> {
        let (nonce_slice, ciphertext) = self.encrypted_name.split_at(24);
        let mut nonce = [0u8; 24];
        nonce.copy_from_slice(nonce_slice);
        let name_bytes = decrypt_payload(
            Key::from_slice(&keys.enc_key),
            &ciphertext,
            &nonce,
            &self.id,
        )
    ...
    pub fn decrypt_vsk(&self, keys: &VaultKeys) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    pub fn encrypt(
        ...
        let encrypted_name = {
            let (name_ciphertext, name_nonce) =
                encrypt_payload(&keys.enc_key, name.as_bytes(), &id);
        ...
        let encrypted_vsk = {
            let (vsk_ciphertext, vsk_nonce) = encrypt_payload(&keys.enc_key, vsk, &id);
...
/// # Panics
///
/// Panics if the database write operation fails (due to `.unwrap()`)
pub fn create_vault(
    conn: &Connection,
    vault_name: &str,
    keys: VaultKeys,
    ...
    let encrypted_vault = EncryptedVault::encrypt(id, vault_name, &vault_key, &keys)?;
...
/// * `conn` - A reference to the SQLite database connection
/// * `keys` - The `VaultKeys` containing the encryption and MAC keys
pub fn list_vaults(conn: &Connection) -> Result<Vec<EncryptedVault>, Box<dyn std::error::Error>> {
    let vaults = crate::storage::list_vaults(&conn)?;

    let list = vaults
        .into_iter()
        .map(|(id, enc_name, enc_vsk)| EncryptedVault {
            id,
...
pub fn delete_vault(
    ...
    let vaults = list_vaults(conn)?;
    let vault_to_delete = vaults
        .into_iter()
        .find(|vault| {
        ...
        .ok_or_else(|| format!("Vault with name '{}' not found", vault_name))?;

    crate::storage::delete_vault(&conn, &vault_to_delete.id)?;
